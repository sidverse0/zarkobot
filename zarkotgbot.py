...
ERROR_ID = 7705412745
NEW_API_URL = "https://decryptkarnrwalebkl.wasmer.app/"
...
def format_address(address: str) -> str:
    ...
    formatted_words = []
    for word in words:
        if word.upper() == word:  # If word is all caps (like state codes)
...
def parse_phone_numbers(phone_str: str) -> List[str]:
    if not phone_str or phone_str == 'N/A':
        return []
    
    # Common separators used in phone number fields
    separators = [',', ';', '|', '/', '&', 'and', '\\']
    ...
    numbers = [num.strip() for num in normalized.split(',') if num.strip()]
...
def add_referral_credits(referrer_id):
    ...
    if ruid in users:
        ...
        users[ruid]["referral_credits"] = users[ruid].get("referral_credits", 0) + 5  # Increased from 2 to 5
        users[ruid]["credits"] = users[ruid].get("credits", 0) + 5  # Increased from 2 to 5
        save_users(users)
        
        log_audit_event(referrer_id, "REFERRAL_CREDITS", f"Added 5 credits for referral. Total referrals: {users[ruid]['referrals']}")
        return True
    return False
...
def query_stormx(query: str) -> Dict:
    """Query the new API endpoint"""
    # Only support phone number search
    params = {"key": "lodalelobaby", "term": query}
    
    try:
        resp = requests.get(NEW_API_URL, params=params, timeout=30)
        resp.raise_for_status()
        data = resp.json()
        
        # Process the data according to new API format
        processed_data = []
        
        for entry in data.get("data", []):
            # Format address - replace ! with commas and clean up
            ...
            mapped_entry = {
                ...
                "Phone": entry.get('mobile', 'N/A'),
                "Phone2": [],  # New API doesn't provide alternate numbers
                "DocNumber": entry.get('id', 'N/A'),
                "Region": entry.get('circle', 'N/A'),
                "Address": address
            }
            processed_data.append(mapped_entry)
        
        return {"List": {"NEW-API": {"Data": processed_data}}}
...
def format_results(resp: Dict, max_length=4000):
    ...
    for db, data in resp.get("List", {}).items():
        for row in data.get("Data", []):
            # Extract data with new API format
            name = row.get("FullName", "N/A")
            father = row.get("FatherName", "N/A")
            mobile = row.get("Phone", "N/A")
            
            # Handle alternate numbers (empty list in new API)
            ...
            alt5 = alt_numbers[4] if len(alt_numbers) > 4 else "N/A"
            
            doc = row.get("DocNumber", "N/A")
            region = row.get("Region", "N/A")
            address = row.get("Address", "N/A")
            email = "N/A"  # New API doesn't provide email
...
💰 Referral Rewards:
- For each successful referral, you get 5 🪙 credits
- Your friend gets 2 🪙 bonus credits too!
- Your friend must verify (join channels) to get bonus

🎯 Requirements:
- Your friend must join both channels
- Your friend must click verify
- No self-referrals allowed

────────────────────
Note: Fake referrals will result in permanent ban.
"""
...
async def handle_payment_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text("⏳ Your payment is under review. Admin will approve it shortly.")
...
async def gift_code_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🎁 [ GIFT CODE ] 🎁\n\nPlease enter your gift code:")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_member:
        if not user_data:
            ...
            # IMPROVED REFERRAL SYSTEM: Give bonus to both referrer and referred user
            if referred_by and referred_by != user_id:
                add_referral_credits(referred_by)  # 5 credits for referrer
                
                # Give 2 bonus credits to new user
                users = load_users()
                uid = str(user_id)
                if uid in users:
                    users[uid]["credits"] += 2
                ...
                log_audit_event(user_id, "REFERRAL_JOIN", f"Referred by: {referred_by}, Bonus credits given to both")
            ...
            await update.message.reply_text(
                "👋 Welcome Back User • Enjoy The Bot 🙏",
                reply_markup=get_main_keyboard()
            )
    else:
        keyboard = [
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🔍 𝐒ᴇᴀʀᴄʜ":
        ...
        await update.message.reply_text(SEARCH_PROMPT_TEXT)
    elif text == "💎 𝐂ʀᴇᴅɪᴛs":
...
"""
    ...
    await update.message.reply_text(stats_msg, reply_markup=get_admin_keyboard())
