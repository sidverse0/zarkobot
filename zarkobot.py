...
def query_stormx(query: str) -> Dict:
    ...
    try:
        ...
        # Process the data to remove duplicates and format addresses
        processed_data = []
        seen_ids = set()
        ...
        for entry in data.get("data", []):
            # Format address
            if 'address' in entry:
                entry['address'] = format_address(entry['address'])
        ...
        for entry in processed_data:
            ...
            searched_number_index = -1
            
            for i, num in enumerate(all_numbers):
                if query in num:
                    searched_number_index = i
                    break
            
            # If we found the searched number, make it the primary
            if searched_number_index >= 0:
                primary_number = all_numbers.pop(searched_number_index)
            else:
                primary_number = mobile_numbers[0] if mobile_numbers else 'N/A'
            
            # Get up to 5 alternate numbers
        ...
        return {"List": {"STORMX-DB": {"Data": mapped_data}}}
...
def format_results(resp: Dict, max_length=4000):
    ...
    for db, data in resp.get("List", {}).items():
        for row in data.get("Data", []):
            ...
            email = "N/A"  # Stormx doesn't provide email
